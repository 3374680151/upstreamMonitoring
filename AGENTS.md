...
11. **接口契约变更必须设计先行**：涉及接口 / 数据库表结构的功能，先设计并经用户确认后才允许写代码；设计稿实时同步 Apifox（见下节「接口设计与 Apifox 同步」）。
...
4. **用户同意合入后 → 合入并自动推送远程**：用户明确表示「验证通过 / 同意合入 master」即视为一次完整授权，agent 随即自动执行合入**并推送远程**，无需用户再单独指示推送：
    ```bash
    git checkout master
    git merge --no-ff <业务名>
    git branch -d <业务名>
    ...
    ```
   推送失败（网络 / 远程拒绝）时如实报告，不静默重试。
5. 功能分支本身默认不推远程（用户要求时才推）。
6. 默认一次只开一个功能分支；上个功能未验证合入前，不要叠开新分支。若验证不通过，回到功能分支修复后再次交给用户验证。确需多个会话并行开发时，按下方「多会话并行（worktree）」执行，**禁止在共享目录里来回 `git checkout` 切分支**。
...
一个工作区同一时刻只能停在一个分支上：两个会话共用同一个目录时，任何一方 `git checkout` 都会把对方连文件带未提交改动一起切走，表现为「来回切分支 / 改动互相冲突」。需要并行时**一个会话一个独立工作区**：

1. 在现有会话里执行 `bash scripts/new_worktree.sh <业务名>`：自动从 master 新建业务分支、在仓库旁建 `../upstream-<业务名>` 独立工作区、复制 `.env`、装好前端依赖（node_modules 不共享，必须各装各的）。
2. 新会话打开 `../upstream-<业务名>` 目录工作；分支、提交、验证环境流程与本目录完全一致，验证端口与主目录错开（如 8020 / 8030）。
3. 所有 worktree 共享同一 `.git` 与本地分支；「一次只开一个功能分支」按工作区计——每个 worktree 同时只跑一个功能分支，合并与推送仍走原流程。
4. 两个任务会改同一批文件时，并行只是把冲突推迟到合并，优先串行；用完清理：`git worktree remove ../upstream-<业务名>` + `git branch -d <业务名>`。
...
├── tests/                       # 已有测试（默认不再新增）
├── scripts/                     # 开发流程辅助脚本（new_worktree.sh：多会话并行建独立工作区）
...
- `.env` 仅承载运行期默认值：DB 连接（含 `DB_CONNECT_TIMEOUT` / `DB_READ_TIMEOUT` / `DB_WRITE_TIMEOUT`）、`DB_POOL_*`、HOST/PORT、APP_TIMEZONE、TZ、CONSOLE_PASSWORD、CONSOLE_SESSION_TTL、SCHEDULER_ENABLED、ENABLE_API_DOCS、SLOW_REQUEST_THRESHOLD_MS、UPSTREAM_HTTP_TIMEOUT、SEED_DEMO、MAIN_CHANNEL_TTFT_FAST_SECONDS / MAIN_CHANNEL_TTFT_SLOW_SECONDS / MAIN_CHANNEL_RECENT_REQUESTS / MAIN_CHANNEL_RECENT_WINDOW_SECONDS / MAIN_CHANNEL_REQUEST_STALE_SECONDS（主站监控「渠道最近请求首字延迟」展示口径）、APIFOX_ACCESS_TOKEN（Apifox Open API 访问令牌，仅本机，用于推送接口设计，见「接口设计与 Apifox 同步」）。
