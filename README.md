...
| `MAIN_CHANNEL_TTFT_FAST_SECONDS` | `10` | 主站监控渠道首字延迟绿色阈值（秒，低于该值绿色） |
| `MAIN_CHANNEL_TTFT_SLOW_SECONDS` | `20` | 主站监控渠道首字延迟红色阈值（秒，达到该值红色，之间橙色） |
...
> ℹ️ 主站监控「最近请求首字」列会实时读取主站管理日志接口（只读）：未命中缓存的渠道按每主站最小间隔串行拉取，渠道很多时首次加载或强刷可能需要数十秒；60 秒内重复访问走缓存秒开。
