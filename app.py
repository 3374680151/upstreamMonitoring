...
def aggregate_newapi_channel_candidates(
    ...
    for channel in channels or []:
        ...
        if channel_id is not None and channel_id not in item["channel_ids"]:
...
def persist_admin_browser_auth_cas(
    ...
    """CAS write of admin site browser auth.

    Only updates ``admin_sites`` rows that still match the validating sync
    request.  System access token, access_user_id and security_proof are
    never touched, even if the request race lost.
    """
    origin = site_origin(expected_origin)
    if not origin or not request_id:
        return False
    ...
    return db_execute_rowcount(
        ...
        (
            ...
            now,
            now,
            int(site_id := int(site.get("id") or 0)),
            str(request_id),
            origin,
        ),
    ) > 0
...
def mark_admin_browser_session_expired_cas(
    admin_site_id: int,
    message: str,
    request_id: str,
    expected_origin: str,
) -> bool:
    """CAS write that marks an admin site sync as expired."""
    origin = site_origin(expected_origin)
    if not origin or not request_id:
        return False
    now = utc_now_iso()
...
def _create_session_sync_request(
    target_kind: str, target_id: int, target: Dict[str, Any]
...
def create_site_session_sync_request(
    ...
    if not site:
        return False, {}, "渠道不存在"
    return _create_session_sync_request("site", int(site_id), site)
...
def create_admin_site_session_sync_request(
    ...
    if not site:
        return False, {}, "管理站不存在"
    return _create_session_sync_request("admin_site", int(admin_site_id), site)
    ...
    with SESSION_SYNC_REQUEST_LOCK:
        ...
        if _session_sync_request_expired(row):
            ...
            if row.get("site_id") is not None:
                ...
                db_execute(
                    """
                    UPDATE admin_sites SET browser_login_last_error = '同步请求已过期',
                        browser_login_last_check_at = ?, updated_at = ?
                    WHERE id = ?
                    """,
    ...
    with SESSION_SYNC_REQUEST_LOCK:
        ...
        if row.get("site_id") is not None and platform == "sub2api":
            ...
            db_execute_rowcount(
                """
                UPDATE admin_sites AS a
                SET browser_login_last_error = CASE WHEN ? = 'ready' THEN NULL ELSE ? END,
                    browser_login_last_check_at = ?, updated_at = ?
                WHERE a.id = ?
                  AND a.platform = 'newapi'
                  AND EXISTS (
...
def _newapi_account_is_admin(account: Dict[str, Any]) -> bool:
    role = account.get("role")
    try:
        return int(role) >= 10
    except (TypeError, ValueError):
        return str(role or "").strip().lower() in {"admin", "administrator", "root", "owner"}
...
def complete_session_sync_request(
    ...
    if platform == "sub2api" and target_kind == "site" and site_id is not None:
        ...
        if (
        ...
        elif target_kind == "admin_site" and (
            not str(session.get("browser_refresh_cookie") or "").strip()
            or not str(session.get("browser_session_id") or "").strip()
        ):
            applied, apply_error = False, "管理站需要完整的 NewAPI 网页登录态"
        else:
            ...
            if applied:
                ...
                # auth-mode change after validation already started.
                sync_error = _newapi_session_sync_request_error(
                    target_kind, target_id, str(request_id), target_origin
                )
                if sync_error:
            ...
            if applied:
                persisted = persist_admin_browser_auth_cas(
                    ...
                    session_id=str(session.get("browser_session_id") or "").strip(),
                    access_expires_at=session.get("browser_access_expires_at") or 0,
                )
                if not persisted:
    ...
    return [
        {
            ...
            "has_browser_session": bool(r.get("browser_access_token") and r.get("browser_session_id")),
            "browser_login_last_error": r.get("browser_login_last_error"),
            "browser_login_last_check_at": r.get("browser_login_last_check_at"),
...
def save_channel_upstream_binding(
    ...
    username = merged_text(
        "login_username",
        same_auth_mode and platform == "sub2api" and auth_mode == "password",
    )
    ...
    if base_url and platform == "newapi" and (not access_token or not access_user_id):
    ...
    with connection.cursor() as cursor:
        ...
        if existing:
            ...
            try:
                ...
                if refreshed and str(
                    refreshed.get("platform") or "newapi"
                ).strip().lower() == "newapi":
...
def reconcile_site_discovery_links(
    ...
    """
    live_pairs = {
        (int(channel_id), normalize_base_url(str(candidate.get("base_url") or "")))
        for candidate in candidates
    ...
    for row in rows:
        try:
            pair = (
                int(row["channel_id"]),
                normalize_base_url(str(row.get("upstream_base_url") or "")),
            )
        except (TypeError, ValueError):
            continue
...
def persist_newapi_site_browser_session(
    site_id: int, session: Dict[str, Any]
) -> None:
    try:
        expires_at = int(session.get("browser_access_expires_at") or 0)
    except (TypeError, ValueError):
...
def refresh_newapi_site_browser_session(
    ...
    with _newapi_site_browser_session_lock(site_id):
        ...
        persist_newapi_site_browser_session(site_id, auth_data)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if admin_sync_status_match:
            ...
            payload = get_admin_site_session_sync_request(
                admin_site_id, request_id
            )
            if payload is None:
                return json_response(
                    self,
        ...
        if path.startswith("/api/admin/sites/"):
            ...
            if len(parts) == 5 and parts[4] == "channel-candidates":
                ...
                source_channels = channels if isinstance(channels, list) else []
                candidates = aggregate_newapi_channel_candidates(source_channels)
                # Reconcile provenance rows only when the upstream read is
                # complete and successful.  Paging failures already returned
                # 502 above, so reaching this point means every page was
                # fully read.
                try:
                    reconcile_site_discovery_links(
                        int(admin_site_id), candidates
                    )
                except Exception:
                    # Reconcile must never break the read-only discovery view;
                    # if the cleanup path fails, the candidates still render.
                    pass
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            admin_sync_create_match = re.fullmatch(
                r"/api/admin/sites/([0-9]+)/session-sync/requests", path
            )
            if admin_sync_create_match:
                admin_site_id = int(admin_sync_create_match.group(1))
            ...
            admin_sync_fail_match = re.fullmatch(
                r"/api/admin/sites/([0-9]+)/session-sync/requests/([A-Za-z0-9_-]{1,64})/fail",
                path,
            )
            if admin_sync_fail_match:
                body = read_json_body(self)
                body = body if isinstance(body, dict) else {}
                admin_site_id = int(admin_sync_fail_match.group(1))
                request_id = admin_sync_fail_match.group(2)
