      ...
      {!sites.length ? (
        ...
        <>
          <div className="mb-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
            <span className="t-micro">合计余额</span>
            <span className="font-serif text-[26px] font-semibold tabular text-ink-strong">
          ...
          <div className="space-y-1.5">
            {sites.map((site) => {
              ...
              return (
                <div
                  key={site.id}
                  className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 rounded-[var(--radius-md)] border border-line bg-panel px-3 py-2"
                >
                  ...
                  <div className="shrink-0 text-[13px]">
                    {row.state === "ok" ? (
                      <span className="font-serif font-semibold tabular text-ink-strong">
                        {usd(row.account.balance_usd)}
                      </span>
                    ) : row.state === "loading" ? (
                      <span className="text-[11.5px] text-ink-muted">查询中…</span>
                    ) : row.state === "error" ? (
                      <span
                        className="text-[11.5px] text-warning-fg"
                        title={row.message}
                      >
                        读取失败
                      </span>
