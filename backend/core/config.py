...
# 主站监控页「渠道最近请求首字延迟」展示口径，全部可在 .env 覆盖（改后需重启）。
MAIN_CHANNEL_TTFT_FAST_SECONDS = _env_int("MAIN_CHANNEL_TTFT_FAST_SECONDS", 10, 1, 600)
# 红色阈值配置颠倒（低于绿色阈值）时向上收敛，避免颜色档位互相穿越。
MAIN_CHANNEL_TTFT_SLOW_SECONDS = max(
    _env_int("MAIN_CHANNEL_TTFT_SLOW_SECONDS", 20, 1, 600),
    MAIN_CHANNEL_TTFT_FAST_SECONDS,
)
MAIN_CHANNEL_RECENT_REQUESTS = _env_int("MAIN_CHANNEL_RECENT_REQUESTS", 10, 1, 50)
MAIN_CHANNEL_RECENT_WINDOW_SECONDS = _env_int("MAIN_CHANNEL_RECENT_WINDOW_SECONDS", 3600, 60, 86400)
MAIN_CHANNEL_REQUEST_STALE_SECONDS = _env_int("MAIN_CHANNEL_REQUEST_STALE_SECONDS", 900, 60, 86400)
